...
public class Camera extends Rectangle {
	 ...
	 * questions asked 2. if an entity is hidden, it is not active 3. if entity's
	 * status is not REMOVED and the entity is not hidden, then there's additional
	 * checks that take place: 1. if entity's isUpdateOffScreen attribute is true,
	 * it is active 2. OR if the camera determines that it is in its boundary range,
	 * it is active
	 */
	private boolean isMapEntityActive(MapEntity mapEntity) {
		return mapEntity.getMapEntityStatus() != MapEntityStatus.REMOVED && !mapEntity.isHidden() && mapEntity.exists()
